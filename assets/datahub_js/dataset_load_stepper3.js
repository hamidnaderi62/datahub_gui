...
//**************************************************
// NER Anonymize - Solution 0 - using ml


// Define your custom NER tags and regex patterns
const NER_RULES = [
    { tag: '[EMAIL]', pattern: /\b[\w.-]+@[\w.-]+\.\w+\b/gi },
    ...
    { tag: '[ADDRESS]', pattern: /\b\d+\s+[\w\s]+,\s*[\w\s]+,\s*[A-Z]{2}\b/g },
    { tag: '[DATE]', pattern: /\b\d{2}\/\d{2}\/\d{4}\b/g },
    { tag: '[URL]', pattern: /\bhttps?:\/\/[^\s]+\b/gi },
];

// Main function to process data
function replaceAllWithNER0(data, column, useML = false) {
    return data.map(item => {
        let processedText = applyNERTags(item[column]);
        if (useML && typeof window !== 'undefined' && window.nlp) {
            processedText = applyNERTagsWithML(processedText);
        }
        return {
            ...item,
...
// Apply regex-based NER tags
function applyNERTags0(text) {
...
// Apply ML-based NER tags (using Compromise.js)
function applyNERTagsWithML0(text) {
    try {
...
function replaceAllWithNER2(data, column) {
